...
<?php 
	...
	jQuery(document).on('ready', function() { 
		jQuery('#dc-calendar-app-".esc_js($flag)."').fullCalendar({
			...
			events:[
				".json_encode($array_date)."
			],
			eventRender: function(event, eventElement) {
				if (event.title == 'current') {
					var dateString = moment(event.start).format('YYYY-MM-DD');
					jQuery('#dc-calendar-app-".esc_js($flag)."').find(\"*[data-date='".$current_date."']\").removeClass('fc-today fc-state-highlight');
					jQuery('#dc-calendar-app-".esc_js($flag)."').find(\"*[data-date='".$appointment_date."']\").addClass('fc-today');
